...
export class TransactionsService {
  ...
  createAdjustment(dto: CreateAdjustmentDto, actorId?: number) {
    return this.execute({
      itemCode: dto.itemCode, type: TransactionType.Adjustment,
      fromLocationId: dto.adjustmentType === AdjustmentType.Decrease ? dto.locationId : undefined,
      toLocationId: dto.adjustmentType === AdjustmentType.Increase ? dto.locationId : undefined,
      qty: dto.qty, adjustmentType: dto.adjustmentType, adjustmentReason: dto.adjustmentReason, notes: dto.notes, actorId,
    });
  }

  private async execute(input: { itemCode: string; type: TransactionType; fromLocationId?: number; toLocationId?: number; qty: number; adjustmentType?: AdjustmentType; adjustmentReason?: AdjustmentReason; customerName?: string; referenceNo?: string; notes?: string; actorId?: number }): Promise<Transaction> {
    const transaction = await this.dataSource.transaction(async (manager) => {
      ...
      if (input.fromLocationId && !fromLocation?.isPhysical) throw new BadRequestException(`${fromLocation?.name ?? 'Source location'} is not a physical location`);
      if (input.toLocationId && !toLocation?.isPhysical) throw new BadRequestException(`${toLocation?.name ?? 'Destination location'} is not a physical location`);
      ...
      const record = manager.create(Transaction, {
        item, transactionType: input.type, fromLocation, toLocation, qty: input.qty,
        adjustmentType: input.adjustmentType ?? null, adjustmentReason: input.adjustmentReason ?? null,
        customerName: input.customerName ?? null, referenceNo: input.referenceNo ?? null, notes: input.notes ?? null,
        stockBefore: snapshots.before, stockAfter: snapshots.after,
      ...
      await manager.save(manager.create(AuditLog, {
        ...
        action: 'create', entity: 'transaction', entityId: String(saved.id),
        newValues: { type: input.type, itemCode: item.code, qty: input.qty, fromLocationId: fromLocation?.id ?? null, toLocationId: toLocation?.id ?? null, adjustmentReason: input.adjustmentReason ?? null },
      }));
    ...
    this.gateway.emitTransactionCreated(transaction.id, transaction.transactionType, transaction.item.code, transaction.qty);
    this.gateway.emitStockUpdated(transaction.item.code, transaction.stockAfter);
    await this.stockAlerts.evaluateAfterTransaction(transaction);

    return transaction;
  }

  private async location(manager: DataSource['manager'], id: number): Promise<Location | null> { return manager.findOne(Location, { where: { id } }); }

  async findAll(query: TransactionSearchDto): Promise<PaginatedResult<Transaction>> {
    ...
    if (query.type) builder.andWhere('transaction.transactionType = :type', { type: query.type });
    if (query.itemCode) builder.andWhere('item.code = :code', { code: query.itemCode.toUpperCase() });
  ...
  async findOne(id: number) {
    const transaction = await this.transactions.findOne({ where: { id }, relations: ['item', 'fromLocation', 'toLocation'] });
