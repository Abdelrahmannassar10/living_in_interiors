...
export interface StockRowSummary {
  locationId: number;
  locationName: string;
  onHand: number;
  reserved: number;
...
const AVAILABLE_SQL =
  'COALESCE(stock_sum.on_hand, 0) - COALESCE(stock_sum.reserved, 0)';
...
export class ItemsService {
  ...
  private withStockAggregates(
    builder: ReturnType<Repository<Item>['createQueryBuilder']>,
  ) {
  ...
  async findAll(query: SearchItemDto): Promise<PaginatedResult<Item>> {
    let builder = this.itemsRepository
    ...
    if (query.q)
      builder.andWhere(
        '(item.code ILIKE :q OR item.description ILIKE :q OR item.category ILIKE :q)',
        { q: `%${query.q}%` },
      );
    if (query.brand)
      builder.andWhere('brand.name ILIKE :brand', {
        brand: `%${query.brand}%`,
      });
    if (query.category)
      builder.andWhere('item.category = :category', {
        category: query.category,
      });
    if (query.inStock === true)
      builder.andWhere(`COALESCE(stock_sum.on_hand, 0) > 0`);
    if (query.inStock === false)
      builder.andWhere('COALESCE(stock_sum.on_hand, 0) = 0');
    const sortColumn =
      query.sortBy === 'unitPrice'
        ? 'item.unit_price'
        : query.sortBy === 'brand'
          ? 'brand.name'
          : 'item.code';
  ...
  async findLowStock(): Promise<Item[]> {
    ...
    builder = this.withStockAggregates(builder);
    builder.andWhere(
      `COALESCE(stock_sum.on_hand, 0) - COALESCE(stock_sum.reserved, 0) <= item.low_stock_threshold`,
    );
  ...
  async getStockSummary(code: string): Promise<StockSummary> {
    ...
    const stocks = await this.dataSource
      ...
      .find({ where: { item: { id: item.id } }, relations: ['location'] });
    const locations: StockRowSummary[] = stocks.map((stock) => ({
      locationId: stock.location.id,
      locationName: stock.location.name,
      onHand: stock.qtyOnHand,
      reserved: stock.qtyReserved,
  ...
  async create(dto: CreateItemDto, actorId?: number): Promise<Item> {
    ...
    if (await this.itemsRepository.findOne({ where: { code } }))
      throw new ConflictException('Item code already exists');
    if (dto.brandId) {
      const brand = await this.dataSource
        .getRepository(Brand)
        .findOne({ where: { id: dto.brandId, isActive: true } });
      ...
      if ((dto.initialQty ?? 0) > 0) {
        if (!dto.initialLocationId)
          throw new BadRequestException(
            'initialLocationId is required when initialQty is provided',
          );
        const location = await manager.findOne(Location, {
          where: { id: dto.initialLocationId },
        });
        if (!location)
          throw new NotFoundException('Initial location not found');
        if (!location.isPhysical)
          throw new BadRequestException(
            `${location.name} is not a physical location`,
          );
        await manager.save(
          manager.create(ItemStock, {
            item,
            location,
            qtyOnHand: dto.initialQty!,
            qtyReserved: 0,
          }),
        );
        await manager.save(
          manager.create(Transaction, {
            ...
            stockBefore: {},
            stockAfter: {
              [String(location.id)]: { onHand: dto.initialQty!, reserved: 0 },
            },
            createdBy: actorId ? { id: actorId } : null,
          }),
        );
  ...
  async update(code: string, dto: UpdateItemDto): Promise<Item> {
    ...
    Object.assign(
      item,
      rest,
      brandId ? { brand: { id: brandId } as Brand } : {},
    );
  ...
  async addPhoto(code: string, file: Express.Multer.File): Promise<ItemPhoto> {
    ...
    const isFirst =
      (await this.dataSource
        .getRepository(ItemPhoto)
        .count({ where: { item: { id: item.id } } })) === 0;
    return this.dataSource.getRepository(ItemPhoto).save(
      this.dataSource.getRepository(ItemPhoto).create({
        item,
  ...
  async deletePhoto(code: string, photoId: number): Promise<void> {
    const item = await this.findOne(code);
    const photo = await this.dataSource
      .getRepository(ItemPhoto)
      .findOne({ where: { id: photoId, item: { id: item.id } } });
  ...
  async setPrimaryPhoto(code: string, photoId: number): Promise<ItemPhoto> {
    ...
    const photo = await repo.findOne({
      where: { id: photoId, item: { id: item.id } },
    });
    if (!photo) throw new NotFoundException('Photo not found');
  ...
  async getPrimaryPhotoUrl(code: string): Promise<string | null> {
    ...
    const photo =
      (await this.dataSource
        .getRepository(ItemPhoto)
        .findOne({ where: { item: { id: item.id }, isPrimary: true } })) ??
      (await this.dataSource
        .getRepository(ItemPhoto)
        .findOne({ where: { item: { id: item.id } }, order: { id: 'ASC' } }));
