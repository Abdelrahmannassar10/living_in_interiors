...
const DEMO_ITEMS: Array<{
  ...
  {
    code: 'SOFA-AURORA',
    description: 'Aurora 3-seater sofa, bouclé',
    category: 'Sofas',
    unitPrice: '2450.00',
    lowStockThreshold: 1,
    showroomQty: 3,
  ...
  {
    code: 'CHAIR-OSLO',
    description: 'Oslo lounge chair, oak frame',
    category: 'Chairs',
    unitPrice: '890.00',
    lowStockThreshold: 2,
    showroomQty: 4,
  ...
  {
    code: 'TABLE-ORY',
    description: 'Ory dining table, walnut',
    category: 'Tables',
    unitPrice: '1700.00',
    lowStockThreshold: 1,
    showroomQty: 1,
...
async function runSeeds(): Promise<void> {
  ...
  if ((await users.count()) === 0) {
    ...
    console.warn(
      'Store this password now — it is not recoverable. You can reset it later via PATCH /users/:id with { password }.',
    );
  }
...
async function seedItems(connection: DataSource): Promise<void> {
  ...
  if (!showroom || !storage1)
    throw new Error('Seeds require Showroom and Storage 1 locations');
    ...
    if (!item) {
      item = await items.save(
        items.create({
          ...
          currency: 'USD',
          lowStockThreshold: demo.lowStockThreshold,
          isActive: true,
          initialQty: demo.showroomQty + demo.storage1Qty,
          qtySold: 0,
        }),
      );
    } else {
...
async function seedNumberSequences(connection: DataSource): Promise<void> {
  ...
  const existing = await sequences.find({
    where: {
      scope: In([
        quotationScope,
        `sales-order:${year}`,
  ...
  if (!existingScopes.has(quotationScope)) {
    const result: Array<{ max_seq: string | null }> = await connection.query(
    ...
    await sequences.save(
      sequences.create({ scope: quotationScope, nextValue }),
    );
  }
