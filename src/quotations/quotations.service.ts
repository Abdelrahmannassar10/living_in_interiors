...
export class QuotationsService {
  constructor(private readonly dataSource: DataSource, @InjectRepository(Quotation) private readonly quotations: Repository<Quotation>, @InjectRepository(QuotationDetail) private readonly details: Repository<QuotationDetail>, @Inject(forwardRef(() => NotificationsService)) private readonly notifications: NotificationsService) {}
  ...
  async updateStatus(id: number, status: QuotationStatus) { const quotation = await this.findOne(id); const valid = (quotation.status === QuotationStatus.Draft && [QuotationStatus.Sent, QuotationStatus.Cancelled].includes(status)) || (quotation.status === QuotationStatus.Sent && [QuotationStatus.Approved, QuotationStatus.Rejected, QuotationStatus.Draft].includes(status)) || (quotation.status === QuotationStatus.Rejected && status === QuotationStatus.Draft) || (quotation.status === QuotationStatus.Approved && status === QuotationStatus.Cancelled); if (!valid) throw new BadRequestException(`Cannot transition from ${quotation.status} to ${status}`); if (status === QuotationStatus.Sent) { if (!quotation.email) throw new BadRequestException('Quotation email is required before sending'); await this.notifications.sendQuotationEmail(id, quotation.email, false); } quotation.status = status; return this.quotations.save(quotation); }
