...
const TRANSITIONS: Record<QuotationStatus, QuotationStatus[]> = {
  ...
  [QuotationStatus.Approved]: [QuotationStatus.Cancelled],
...
export class QuotationsService {
  ...
  private async allocateQuoteNo(manager: DataSource['manager']): Promise<string> {
    ...
    return this.numbering.next(
      ...
      async () => {
        const result = await manager.query(`SELECT COALESCE(MAX(CAST(SUBSTRING(quote_no FROM 'Q\\.AR#([0-9]+)-') AS INTEGER)), 0) AS max_seq FROM quotations WHERE quote_no LIKE $1`, [`Q.AR#%-${year}`]);
  ...
  async findAll(query: PaginationDto): Promise<PaginatedResult<Quotation>> {
    const builder = this.quotations.createQueryBuilder('quotation')
  ...
  async findOne(id: number) {
    const quotation = await this.quotations.findOne({ where: { id }, relations: ['client', 'details', 'details.item'] });
    if (!quotation) throw new NotFoundException('Quotation not found');
    quotation.details = quotation.details.filter((detail) => !detail.isDeleted).sort((a, b) => a.sortOrder - b.sortOrder);
    return quotation;
  }

  async create(dto: CreateQuotationDto, actorId?: number) {
    return this.dataSource.transaction(async (manager) => {
      const client = dto.clientId ? await manager.findOne(Client, { where: { id: dto.clientId, isActive: true } }) : null;
  ...
  async updateStatus(id: number, status: QuotationStatus) {
    ...
    if (!allowed.includes(status)) throw new BadRequestException(`Cannot transition from ${quotation.status} to ${status}`);
    if (status === QuotationStatus.Sent && !quotation.email) throw new BadRequestException('Quotation email is required before sending');
   ...
   */
  async recordRevision(manager: DataSource['manager'], quotation: Quotation, changedBy: number | null, changeSummary: string): Promise<void> {
    const details = await manager.find(QuotationDetail, { where: { quotation: { id: quotation.id }, isDeleted: false }, order: { sortOrder: 'ASC' } });
    const snapshot = {
      quoteNo: quotation.quoteNo, clientName: quotation.clientName, status: quotation.status,
      discountGlobal: quotation.discountGlobal, vatPercent: quotation.vatPercent, currency: quotation.currency,
      totals: computeQuotationTotals({ lines: details, discountGlobalPercent: quotation.discountGlobal, vatPercent: quotation.vatPercent, currency: quotation.currency }),
      lines: details.map((detail) => ({ code: detail.codeSnapshot, qty: detail.qty, unitPrice: detail.unitPrice, discountPercent: detail.discountPercent, totalPriceAfterDiscount: detail.totalPriceAfterDiscount })),
    };
    quotation.revision += 1;
    await manager.save(quotation);
    await manager.save(manager.create(QuotationRevision, { quotation, revisionNumber: quotation.revision, snapshot, changedBy: changedBy ? { id: changedBy } : null, changeSummary }));
  }

  computeTotals(id: number) {
    return this.findOne(id).then((quotation) => computeQuotationTotals({
