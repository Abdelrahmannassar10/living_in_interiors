...
describe('Golden path (e2e)', () => {
  ...
  async function setupFixtures(ds: DataSource): Promise<void> {
    ...
    for (const loc of locationNames) {
      if (!(await locationRepo.findOneBy({ name: loc.name }))) await locationRepo.save(locationRepo.create(loc));
    }
    ...
    if (!admin) {
      await userRepo.save(userRepo.create({ username: 'e2e_admin', password, fullName: 'E2E Admin', role: Role.Admin, isActive: true }));
    }
    const manager = await userRepo.findOneBy({ username: 'e2e_manager' });
    if (!manager) {
      await userRepo.save(userRepo.create({ username: 'e2e_manager', password, fullName: 'E2E Manager', role: Role.Manager, isActive: true }));
    }
  ...
  it('records a sale of 1 unit', async () => {
    const res = await request(app.getHttpServer())
      ...
      .set('Authorization', `Bearer ${adminToken}`)
      .send({ itemCode, fromLocationId: 1, qty: 1, customerName: 'E2E Customer' })
      .expect(201);
  ...
  it('creates a quotation and adds + edits a line', async () => {
    const create = await request(app.getHttpServer())
      .post(`${api}/quotations`)
      .set('Authorization', `Bearer ${adminToken}`)
      .send({ clientName: 'E2E Client', email: 'client@example.com', projectName: 'Golden Project', currency: 'USD' })
      .expect(201);
  ...
  it('sends a quotation (status Draft → Sent)', async () => {
    const create = await request(app.getHttpServer())
      .post(`${api}/quotations`)
      .set('Authorization', `Bearer ${adminToken}`)
      .send({ clientName: 'Send Client', email: 'send@example.com', projectName: 'Sendable', currency: 'USD' })
      .expect(201);
    const quotationId = create.body.data.id;
    const res = await request(app.getHttpServer())
  ...
  it('denies quotation status changes to Staff (RBAC)', async () => {
    ...
    const password = await bcrypt.hash('Admin1234test', 12);
    const staff = await ds.getRepository(User).findOneBy({ username: 'e2e_staff' });
    if (!staff) {
      await ds.getRepository(User).save(ds.getRepository(User).create({ username: 'e2e_staff', password, fullName: 'E2E Staff', role: Role.Staff, isActive: true }));
    }
    const login = await request(app.getHttpServer()).post(`${api}/auth/login`).send({ username: 'e2e_staff', password: 'Admin1234test' }).expect(201);
    const staffToken = login.body.data.accessToken;

    await request(app.getHttpServer())
      .post(`${api}/quotations`)
      .set('Authorization', `Bearer ${staffToken}`)
      .send({ clientName: 'Staff Quot', email: 'staff@example.com', currency: 'USD' })
      .expect(403);
  });
